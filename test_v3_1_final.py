"""
Test V3.1 FINAL - Configuration 6 alertes/jour implémentée
"""
...
# V3.1 FINAL (6 alertes/jour)
V3_1_FINAL = {
    'MIN_TOKEN_AGE': 0.0,
    'MIN_VELOCITE_DEFAULT': 5.0,
    'DANGER_AGE_MIN': 12.0,
    'DANGER_AGE_MAX': 24.0,
...
def main(json_file):
    print(f"\n{'='*80}")
    print(f"   TEST V3.1 FINAL - Configuration 6 alertes/jour")
    print(f"{'='*80}\n")
    ...
    passed = [a for a in alerts if passes_filters(a, V3_1_FINAL)]
    ...
    if 5.5 <= per_day <= 6.5:
        print(f"  [OK] Objectif 6/jour atteint ({per_day:.1f}/jour)")
    elif per_day < 5.5:
        print(f"  [!] Sous objectif ({per_day:.1f}/jour < 6/jour)")
    else:
        print(f"  [+] Au-dessus objectif ({per_day:.1f}/jour > 6/jour)")
    ...
    if roi >= 4:
        print(f"  [OK] ROI acceptable ({roi:+.1f}%/mois >= 4%)")
    else:
        print(f"  [!] ROI faible ({roi:+.1f}%/mois < 4%)")

    if 5.5 <= per_day <= 6.5 and avg_score >= 87 and roi >= 4:
        print(f"\n  => CONFIGURATION VALIDEE - Prete pour deploiement!")
    elif per_day >= 5 and avg_score >= 85:
        print(f"\n  => CONFIGURATION ACCEPTABLE - Deploiement possible")
    else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_v3_1_final.py alerts_railway_export_utf8.json")
        sys.exit(1)
